...
def get_tile_position(zoom, x, y):
    ...
    left = -180.0 + (x-1) * (360.0/column_count)
    up = 90.0 - (y-1) * (180.0/row_count)
    return (left, up, left + 360.0/column_count, up - 180.0/row_count)
...
def random_point(left_border, up_border, right_border, down_border, fid):
    ...
    point["coordinate"] = (left_border + random.random()* wide, 

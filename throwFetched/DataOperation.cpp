        ...
        const BBOX& boundingBox, const charStream& attrColumn)
{
    ...
    OGRFeature *pfetchFeature = NULL;
    OGRGeometry *pgeom = NULL;
    char * wktGeom = NULL;
    while(true)
    {
        pfetchFeature = poPgLayer->GetNextFeature();
        if(NULL == pfetchFeature)
            break;
        std::cout<<pfetchFeature->GetFID()<<std::endl;
        pgeom = pfetchFeature->GetGeometryRef();

        pgeom->exportToWkt(&wktGeom);
        std::cout<<wktGeom<<std::endl;

        delete pfetchFeature;
        pfetchFeature = NULL;
